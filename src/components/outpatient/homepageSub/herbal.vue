<template>
  <div id="herbal">
    ...
    <add-herbal ref="addHerbal"></add-herbal>
  </div>
</template>

<script>
  //局部注册组件
  import addHerbal from '../dialog/addHerbal'
  ...
  export default {
    ...
    mounted: function () {
      ...
      var that = this;
      this.$axios.post('/api/outpatient/getPreH', {
        uRid: this.uRid,
        regid: this.regid,
      })
    ...
    methods: {
      ...
      addHerbalicineDialog() {
        this.$refs.addHerbal.preid =this.currentRow.preid;
        this.$refs.addHerbal.tempobject=this.currentRow;
        this.$refs.addHerbal.itemcode='';
        this.$refs.addHerbal.medicine=[];
        this.$refs.addHerbal.number=1;
        this.$refs.addHerbal.dialogVisible = true;
      },
      refreshPre: function () {
        var that = this;
        this.$axios.post('/api/outpatient/getPreH', {
